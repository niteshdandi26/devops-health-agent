          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {latestCheck?.analysis && !loading && (
              <div className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 shadow-xl hover:shadow-2xl transition-all duration-300">
                ...
                <div className="bg-black/50 rounded-xl p-4 mb-4 font-mono text-sm text-green-400 overflow-x-auto">
                  <code>
                    {`# Fix for: ${latestCheck.analysis.error}
# Generated by AI Agent at ${new Date(latestCheck.timestamp).toLocaleString()}

# Step 1: Backup current configuration
cp /etc/app/config.yml /etc/app/config.yml.backup

# Step 2: Apply recommended fix
${latestCheck.analysis.solution.split('\n')[0] || 'Apply the solution described above'}
...
tail -f /var/log/app/error.log`}
                ...
                <div className="flex gap-3 flex-wrap">
                  <button
                    onClick={() => {
                      const fixScript = `# Fix for: ${latestCheck.analysis.error}
...
sudo systemctl restart app-service`;
                      ...
                      toast.success('Fix commands copied to clipboard!');
                    }}
                    className="px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl font-medium hover:shadow-xl transition-all flex items-center gap-2"
                  >
                    📋 Copy Commands
                  </button>
                  <button
...
echo "Applying fix for: ${latestCheck.analysis.error}"
...
echo "Fix applied successfully!"`;
                      ...
                      a.href = url;
                      a.download = 'fix-script.sh';
                      a.click();
                      URL.revokeObjectURL(url);
                      toast.success('Fix script downloaded!');
                    }}
                    className="px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-xl font-medium hover:shadow-xl transition-all flex items-center gap-2"
                  >
                    💾 Download Script
                  </button>
                </div>
